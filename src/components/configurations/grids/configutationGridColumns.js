...
export const columns = ({nomRefs, configurations, domain, code}) => [
    ...
    { field: 'libelle', headerName: 'Libellé', flex: 3, renderCell },
    ...
    { field: 'timestamp', headerName: 'Période de validité', flex: 2, renderCell, valueGetter: ({row}) => (`${convertDate(row?.dateDebut)}-${convertDate(row?.dateFin)}`) },
