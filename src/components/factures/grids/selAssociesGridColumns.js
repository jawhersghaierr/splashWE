...
export const columns = ({handleModalOpen}) => [
    ...
    { field: 'statut', headerName: 'Statut', flex: 2, renderCell, valueGetter: (params) =>  ( <Chip label={`${params?.value?.libelle}`} sx={{color: 'black', bgcolor: rocStatus[params.value?.code]?.color || 'rgba(0, 0, 0, 0.08)'}}/> ) },
    { field: 'beneficiaire', headerName: 'Nom et date de naissance beneficiaire', flex: 3, renderCell, valueGetter: ({value}) => {
