...
export const columns = ({nomRefs}) => [
    ...
    { field: 'finessGeographique', headerName: 'FINESS géographique', flex: 1, sortable: false, renderCell },
    ...
    { field: 'numeroAmc', headerName: 'AMC', flex: 1, sortable: false, renderCell },
    { field: 'numeroAdherant', headerName: 'N° adhérent', flex: 1, sortable: false},
    { field: 'nom', headerName: 'Nom et date de naissance bénéficiaire', flex: 1, minWidth: '200px', sortable: false, renderCell, valueGetter: ({row}) => (<span><b>{row?.nom}</b>&nbsp;{row?.prenom}<br/>{dateConvertNaissance(row?.dateNaissance)}</span>) },
...
export const reverseMapRocEnLigne = {
    ...
    type: 'Type',
    domaine: 'Domaine',
    finessGeographique: 'FINESS géographique',
    ...
    dateNaissance: 'Nom et date de naissance bénéficiaire',
