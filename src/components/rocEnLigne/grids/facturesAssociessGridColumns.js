...
export const columns = ({nomRefs, handleModalOpen}) => [
    { field: 'dateTimeReceived', headerName: 'Reçu le', minWidth: '150px', flex: 2, renderCell, valueGetter: ({value}) => convertDate(value, true) },
    { field: 'numeroFacture', headerName: 'N° facture', flex: 2, renderCell},
    ...
    { field: 'nom', headerName: 'Nom et date de naissance bénéficiaire', flex: 2, minWidth: '200px', renderCell, valueGetter: ({row}) => (<span><b>{row?.nom}</b>&nbsp;{row?.prenom}<br/>{dateConvertNaissance(row?.dateNaissance)}</span>) },
    { field: 'montantRc', headerName: 'Montant RC', flex: 1, valueGetter: ({ value }) => currencyFormatter.format(value), cellClassName: 'boldValue'},
