...
export const columns = nomRefs => [
    ...
    { field: 'numeroVirement', headerName: 'N° virement', flex: 1, width: 175, type: "number", renderCell },
    { field: 'numeroDecompte', headerName: 'N° décompte', flex: 1, minWidth: '175px', type: "number", renderCell },
    { field: 'numeroPsAPayer', headerName: 'Nº PS à payer', flex: 2, minWidth: '175px', renderCell },
    { field: 'iban', headerName: 'IBAN\nBIC', flex: 3, sortable: false, renderCell, valueGetter: ({value, row}) => (<span>{value}<br/>{row?.bic}</span>) },
    { field: 'montantVirement', headerName: 'Montant', type: 'number', flex: 1, renderCell, valueGetter: ({ value }) => currencyFormatter.format(value), cellClassName: 'boldValue'},
