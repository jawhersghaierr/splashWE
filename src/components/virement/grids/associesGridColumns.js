...
export const columns = ({nomRefs, handleModalOpen}) => [
    ...
    { field: 'status', headerName: 'Statut', flex: 2, renderCell, valueGetter: ({value}) => (<Chip label={`${nomRefs.PAIEMENT_STATUS[value]}`} sx={{color: 'black', bgcolor: paiementsStatus[value].color}}/> ) },
